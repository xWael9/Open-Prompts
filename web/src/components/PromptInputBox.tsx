...
const PromptInputAction: React.FC<React.ComponentProps<typeof Tooltip> & { tooltip: React.ReactNode; children: React.ReactNode; side?: "top" | "bottom" | "left" | "right" }> = ({
...
export const PromptInputBox = React.forwardRef((props: PromptInputBoxProps, ref: React.Ref<HTMLDivElement>) => {
  const { onSend = () => {}, isLoading = false, placeholder = "Describe the prompt you need...", className } = props;
  ...
  const handleSubmit = () => {
    if (input.trim() || files.length > 0) {
      let mode = "";
      if (showSearch) mode = "search";
      else if (showThink) mode = "think";
      else if (showCanvas) mode = "canvas";
      const formattedInput = mode ? `[${mode}] ${input}` : input;
    ...
    <>
      ...
      >
        ...
        <div className={cn("transition-all duration-300", isRecording ? "h-0 overflow-hidden opacity-0" : "opacity-100")}>
          <PromptInputTextarea
            placeholder={showSearch ? "Search prompt templates..." : showThink ? "Think deeply about prompt design..." : showCanvas ? "Describe your prompt canvas..." : placeholder}
            className="text-base"
          />
        </div>
